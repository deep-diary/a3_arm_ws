...
def generate_arm_nodes(context, *args, **kwargs):
    ...
    if not config_file:
        config_file = os.path.join(
            FindPackageShare('a3_description').perform(context),
    ...
    for arm_name, arm_cfg in arms_config.items():
        ...
        pkg_share = FindPackageShare('a3_description').perform(context)
    ...
    if use_rviz.lower() == 'true':
        rviz_config_file = PathJoinSubstitution([
            FindPackageShare("a3_description"), "config", "el_a3_view.rviz"
        ])
